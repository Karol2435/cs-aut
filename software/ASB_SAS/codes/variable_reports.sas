...
block GRP_&zm / discrete group=&period sumvar=n_PctSum_01_n type=sum 
patternid=group subgroup=n_Sum;
label n_Sum='Number of accounts' GRP_&zm='GRP'
n_PctSum_01_n='Number percent';
format n_PctSum_01_n procent. n_sum 12.;
run;
quit;
...
title3 "&label";
proc gchart data=stat_am;
block GRP_&zm / discrete group=&period sumvar=all_PctSum_01_all type=sum 
patternid=group subgroup=all_Sum;
label all_Sum='Balance' GRP_&zm='GRP'
all_PctSum_01_all='Balance percent';
format all_PctSum_01_all procent. all_sum liczba.;
run;
quit;
...
type=sum 
patternid=group subgroup=PctSum;
label PctSum='Limit percent' GRP_&zm='GRP'
credit_limit_sr_Sum='Average credit limit';
format PctSum procent. credit_limit_sr_Sum liczba.;
run;
quit;
