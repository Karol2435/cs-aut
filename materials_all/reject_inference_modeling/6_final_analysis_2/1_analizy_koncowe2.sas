...
%let kat_kody=c:\karol\analiza6_poziom_klienta\project\students\11111\modelowanie_rj\6_analizy_koncowe2\;
...
%let zbior=wej.abt;
%include "c:\karol\analiza6_poziom_klienta\project\students\11111\modelowanie_rj\5_model_all2\validacja\data\kod_do_skorowania.sas" / source2;

data wej.abt;
set wej.abt_score;
rename SCORECARD_POINTS=score_new_rj2;
drop psc:;
run;
...
/*new rj ind*/
proc logistic data=gini;
model default12_ind=score_new_rj2;
run;
proc logistic data=gini;
model default12_ind=score_new_rj2;
where decision='A';
...
if _n_=13 then do; des='New score rj2'; type='default12'; dataset='All'; end;
if _n_=14 then do; des='New score rj2'; type='default12'; dataset='Accepted'; end;
if _n_=15 then do; des='New score rj2'; type='default12'; dataset='Rejected'; end;

if _n_=16 then do; des='New score rj2'; type='default12_ind'; dataset='All'; end;
if _n_=17 then do; des='New score rj2'; type='default12_ind'; dataset='Accepted'; end;
if _n_=18 then do; des='New score rj2'; type='default12_ind'; dataset='Rejected'; end;
...
/*kalibracje*/
libname kal 'c:\karol\analiza6_poziom_klienta\project\students\11111\modelowanie_rj\5_model_all2\validacja\data\';
...
data test2;
set test;
pd_ins_new_rj2=1/(1+exp(-(-0.034903659*SCORECARD_POINTS+10.479516273)));
run;
...
pd_ins_new_rj2=1/(1+exp(-(-0.034903659*score_new_rj2+10.479516273)));
if decision='A' then do;
	naive_rj_new=Default12;
	naive_rj_old=Default12;
	naive_rj=Default12;
	correct_rj_new2=Default12;
end; else do;
	naive_rj_new=pd_ins_new;
	naive_rj_old=pd_ins_old;
	naive_rj=pd_ins;
	correct_rj_new2=pd_ins_new_rj2;
end;
run;
...
var default12 naive_rj_new naive_rj_old naive_rj correct_rj_new2;
...
naive_rj_old='RJ old'
naive_rj='RJ all'
correct_rj_new2='Correct RJ new2'
...
var score_new score_old score_new_rj2;
ranks score_nband score_oband score_nrjband2;
run;
...
naive_rj_old='RJ old'
naive_rj='RJ all'
correct_rj_new2='RJ correct2'
pd_ins_new_rj2='PD correct2'
)*mean=''*f=nlpct12.2*(decision='' all)
/ box='New score bands'
;
run;
...
title "Score bands based on new rj scroe2";
proc tabulate data=kal_ranks;
class score_nrjband2 decision;
var score_new_rj2 default12 naive_rj_new naive_rj_old naive_rj
correct_rj_new2 pd_ins_new_rj2;
table score_nrjband2='' all, 
score_new_rj2='Score limits'*(min='Min' mean='Avg' max='Max')*f=12.
colpctn='Pct'*f=procent.*(decision='' all)
(default12='Risk' 
naive_rj_new='RJ new' 
naive_rj_old='RJ old'
