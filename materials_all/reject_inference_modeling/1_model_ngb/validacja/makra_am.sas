...
block band / discrete group=&period sumvar=percent type=sum 
patternid=group subgroup=all;
label all='Balance' band='Score band'
...
proc gchart data=stat_am;
block band / discrete group=&period sumvar=sr type=sum 
patternid=group subgroup=number;
label number='Number of accounts' band='Score band'
sr='Average balance';
...
title3 "Variable description: &&o&i";
proc gchart data=stat_am;
block &&v&i / discrete group=&period sumvar=percent type=sum 
patternid=group subgroup=all;
label all='Balance' &&v&i='Partial score'
percent='Balance percent';
format percent nlpct12.2 all liczba.;
run;
quit;
...
title3 "Variable description: &&o&i";
proc gchart data=stat_am;
block &&v&i / discrete group=&period sumvar=sr type=sum 
patternid=group subgroup=number;
label number='Number of accounts' &&v&i='Partial score'
sr='Average balance';
format all sr liczba.;
run;
quit;
...
title3 "Variable description: &&o&i";
proc gchart data=stat_am;
block &&v&i / discrete group=&period sumvar=Bad_rate type=sum 
patternid=group subgroup=bad axis=0 to &cent by 0.02;
label bad='Balance for bads' &&v&i='Partial score'
bad_rate="&measure";
format bad_rate nlpct12.2 bad liczba.;
run;
quit;
...
/*proc sql noprint;*/
/*insert into raporty_spis*/
/*values('Bad rate end',"&title for &risk_tar","&title._index_bad_rate_end_&risk_tar._&state..html");*/
/*quit;*/
/**/
/*goptions reset=all device=activex;*/
/*ods listing close;*/
/*ods html path="&dir" (url=none) */
/*frame="&title._index_bad_rate_end_&risk_tar._&state..html"*/
/*body="&title._b_bad_rate_end_&risk_tar._&state..html" */
/*contents="&title._c_bad_rate_end_&risk_tar._&state..html"*/
/*style=statistical;*/
/*%bad_rate_am(Quarter,&risk_tar,Outstanding_&risk_tar,end of outcome period);*/
/*%bad_rate_vars_am(Quarter,&risk_tar,Outstanding_&risk_tar,end of outcome period);*/
/*%bad_rate_am(Month,&risk_tar,Outstanding_&risk_tar,end of outcome period);*/
/*%bad_rate_vars_am(Month,&risk_tar,Outstanding_&risk_tar,end of outcome period);*/
/*ods html close;*/
/*ods listing;*/
/*goptions reset=all device=win;*/
/**/
